...
getGOresults = function(geneset, genereference, organism = "mmusculus",
                        sources=c("GO:BP", "GO:MF", "GO:CC", "KEGG", "TF",
                                  #"MIRNA",
                                  "CORUM", "HP", "HPA"), 
                        custombckgrnd="yes", showgenelist=F){
  
  if(custombckgrnd=="yes"){
    
    resgo = gost(geneset, organism =organism,
                 correction_method = "gSCS",
                 domain_scope = "custom",
                 sources = sources,
                 evcodes = showgenelist,
...
nicetable<- function(dataframe, caption=NULL){
  numidx=sapply(dataframe[1,], is.numeric)
  ...
  if(length(dataframe)>0){
    datatable(dataframe, 
      ...
      formatSignif(columns = colnames(dataframe)[numidx], digits = 3)
  }else{
    datatable(data.frame(), caption = "no significant results")
  }
}
  ...
  fluidPage(
    ...
    sidebarLayout(
      ...
      sidebarPanel(width = 2,
                   ...
                   selectInput("geneunivers", "Enrichment: use detected miRNAs as gene set (recommended)", 
      ...
      mainPanel(width = 10,
                tabsetPanel(
                  ...
                  tabPanel("Network Analysis", 
                           fluidRow("", column(3, selectInput("showlabels", "show miRNA names", 
                                                              choices = c("yes", "no"), 
                                                              selected="no"))
                                    ),
                           fluidRow("", 
                                    column(6, visNetworkOutput("Network",height="700px")), 
                                    column(6, DT::dataTableOutput("table_GO_Network"))), 
                           fluidRow("", 
                                    column(9, p("miRNA size = significance, Width of connection = correlation between two miRNAs"))
...
server <- function(input, output, session){
  ...
  observe({
    ...
    req(inFileReads)
    tryCatch(eval(ReadsRaw <-read.xlsx(inFileReads$datapath, sheet = "miRNA_piRNA")), 
             warning = function(w) {
    ...
    tryCatch(eval(ReadsRaw <- column_to_rownames(ReadsRaw, "miRNA")),
             warning = function(w) {
               showNotification(w$message,type = "warning")
               return(NULL)
             }, error = function(e) {
               showNotification(paste('there was an error reading the Reads file:', e$message),'',type = "error")
               return(NULL)
             }, silent=TRUE)
  ...
  observe({
    metaRaw <- myvalues[["metaRaw"]]
    CaCo=c()
    tryCatch(eval(CaCo <- unique(metaRaw[,input$target])),
             warning = function(w) {
               showNotification(w$message,type = "warning")
               return(NULL)
             }, error = function(e) {
               showNotification(paste('there was an error reading the meta file:', e$message),'',type = "error")
               return(NULL)
             }, silent=TRUE)
    updateSelectInput(session, 
                      inputId = "treatment",
  ...
  observeEvent(input$run_now, {
    withProgress(message = "analyzing...", {
      ...
      req(meta)
      Samples <- intersect(colnames(ReadsRaw), rownames(meta))
      if(length(Samples)==0){
      ...
      ## check metafiles  ####
      tryCatch(eval( meta$group <- as.factor(meta[,input$target])),
               error=function(e){showNotification(e$message, type="error")})
      req(meta$group)
      ...
      if(myvalues$sigNum>=input$minInNetwork){
        ...
        showNotification(paste0("less then ", input$minInNetwork, " miRNAs were significant after adjustment for multiple correction. 
      ...
      if(length(myvalues$targetmirs)>1){
        ...
        if(sum(CorMAT)>0){
          ...
          if(length(unique(meta$group))==2){
            ...
            V(graph)$frame.color=cols
            V(graph)$frame.width=2
          ...
          n=10
          V(graph)$size=n+minmaxscale(-log10(resSig$pvalue))*n
          
          resSig$cluster=membership(CEB)
          
          plab=formatC(resSig$padj,format = "e", digits = 1)
          mirname=V(graph)$name
          V(graph)$name= paste("<p><strong>",V(graph)$name, "</strong><br>p.adj: ", plab, 
                               ", log2FC: ", formatC(resSig$log2FoldChange, digits = 2), "</p>") 
        ...
        } else{ myvalues$graph = make_empty_graph()
        showNotification("no meaning ful networks detected, consider reducing the minimum correlation value", type="warning")
        }
      } else {
        showNotification("to few significant hits for Network calculation", type="warning")
      ...
      mirsreg <- grep("miR|let", rownames(myvalues$restab_DEseq_sig), 
                      value = T)
      
      genes_mirreg <- foreach(i=filtgenedict[mirsreg], 
                              .combine=c) %do% (rownames(i)) %>% unique()
      mirsup <- grep("miR|let", rownames(myvalues$restab_DEseq_sig[myvalues$restab_DEseq_sig$log2FoldChange>0,]), 
                     value = T)
      genes_mirup <- foreach(i=filtgenedict[mirsup], 
                             .combine=c) %do% (rownames(i)) %>% unique()
      
      mirsdwn <- grep("miR|let", rownames(myvalues$restab_DEseq_sig[myvalues$restab_DEseq_sig$log2FoldChange<0,]), 
                      value = T)
      ...
      if(length(myvalues$genes_mirreg)>0){
        myvalues$ResGO_all = getGOresults(myvalues$genes_mirreg,
                                          ...
                                          input$organism, 
                                          custombckgrnd = input$geneunivers)} else {myvalues$ResGO_all=NULL}
      
      if(length(myvalues$genes_mirup)>0){
        myvalues$ResGO_up = getGOresults(myvalues$genes_mirup,
                                         myvalues$geneunivers,
                                         input$organism,
                                         custombckgrnd = input$geneunivers)} else {myvalues$ResGO_up=NULL}
      
      if(length(myvalues$genes_mirdwn)>0){
        myvalues$ResGO_dwn = getGOresults(myvalues$genes_mirdwn,
                                          myvalues$geneunivers,
                                          input$organism, 
                                          custombckgrnd = input$geneunivers)} else {myvalues$ResGO_dwn=NULL}
      
      
      
      tmpgenelist=list()
      
      if(length(resSig$cluster)>0){
        for(cl in sort(unique(resSig$cluster))){
          tmpgenelist[[paste0("cluster_",cl)]]<- foreach(i=filtgenedict[grep("miR|let",rownames(resSig)[resSig$cluster==cl], value=T)], 
                                                         .combine=c) %do% (rownames(i)) %>% unique()
        }
        myvalues$clusterGO=getGOresults(tmpgenelist,
                                        myvalues$geneunivers,
                                        input$organism, 
                                        custombckgrnd = input$geneunivers)
      }
  ...
  observe({
    ...
    output$table_display_samples <- renderDataTable(withProgress(message = "generating ...", 
                                                                 {
                                                                   ...
                                                                   datatable(f, extensions = "Buttons",
                                                                     ...
                                                                     formatSignif(columns = c('totalReads'), digits = 3)
                                                                 }))
    ...
    output$summary <- renderUI(withProgress(message = "generating ...", {
      if(input$geneunivers == "yes"){
        refgenestext = "In enrichment analyes the geneuniverse was defined as the set of genes predicted to be targeted by all miRNAs tested in this study (custom background)."
      } else {
        refgenestext = "All annotated genes were considered as geneunivers in enrichment analyses."
      }
      ...
      HTML(paste0(
        ...
        myvalues$GraphAnal," &#8804; ", input$p.cut, ". </p>
                ...
                Coverage files were converted into raw counts matrices using the Qiagen pipeline. <br>
    ...
    output$Network <- renderVisNetwork(
      withProgress(message = "generating ...", {
        ...
        if(length(V(g))>1){
          ...
          if(input$showlabels=="yes"){
            V(g)$title= V(g)$name
            V(g)$name= gsub("<br>", "\n", gsub("<p>|</p>|<strong>| |</strong>","",V(g)$name))
          } else {
            V(g)$title= V(g)$name
          }
          
          
          visG = toVisNetworkData(g, idToLabel = input$showlabels=="yes") 
          
          visG$nodes$value=visG$nodes$size*3
          visG$nodes$color.border=visG$nodes$frame.color
          set.seed(42)
          visNetwork(visG$nodes, 
                     visG$edges, 
                     main=list(text=paste("Network of small RNAs with",
                                          myvalues$GraphAnal, "< ", 
                                          input$p.cut), 
                               style="font-family:sans-serif;font-weight:bold;font-size:20px;text-align:center;"), 
                     randomSeed = 42)  %>% visIgraphLayout()
          ...
          visNetwork(nodes, edges, main=list(text="not enough significant hits or connections", 
                                             style="font-family:sans-serif;font-size:14px;text-align:center;")) %>% 
            visNodes(hidden = T)
    ...
    output$table_display_fc <- renderDataTable(
      withProgress(message = "generating ...", 
                   ...
                   incProgress(1/2)
                   nicetable(dataset, caption="Differential gene expression results")
                   }), server = F)
    ...
    ## table_GO_Network ####
    output$table_GO_Network <- renderDataTable(
      withProgress(message = "generating ...", {
        
        
        plotdata=as.data.frame(myvalues$clusterGO$result)
        if(length(plotdata)>0){
          clusters=paste0("cluster_", sort(unique(myvalues$restab_DEseq_sig$cluster)))
          nicetable(plotdata, caption = "Network clusters GO term anaylsis") %>% formatStyle("query" ,backgroundColor =styleEqual(clusters, AGCcol[1:length(clusters)]))
        } else {
          nicetable(plotdata, caption = "No results available")
        }
      }
      ))
    ...
    output$table_GO_all <- renderDataTable(
      withProgress(message = "generating ...", 
                   {
                     plotdata=as.data.frame(myvalues$ResGO_all$result)
                     nicetable(plotdata, caption = "GO terms targets of all regulated miRNAS")
                   }))
    ## GO_plot_up ####
    print("GO_plot_up")
    ...
    output$table_GO_up <- renderDataTable(
      withProgress(message = "generating ...", 
                   { plotdata=as.data.frame(myvalues$ResGO_up$result)
                   nicetable(plotdata, caption = "GO terms targets of all up-regulated miRNAS")
                   }))
    ...
    output$table_GO_dwn <- renderDataTable(
      withProgress(message = "generating ...", 
                   {plotdata=as.data.frame(myvalues$ResGO_dwn$result)
                   nicetable(plotdata, caption = "GO terms targets of all down-regulated miRNAS")
                   }))
    print("done")
